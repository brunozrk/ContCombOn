...
public class ContactActivity extends BaseActivity {
    ...
    private Handler handler = new Handler() {
        @Override
         public void handleMessage(Message msg) {
        	...
        	switch (msg.what) {
        		case CREATE_SUCCESS:  util.showToast(getResources().getString(R.string.send_success), 
        		...
        		case LIST:				progressbar.setVisibility(View.GONE);
        								...
        								break;
        		case EDIT_FORM:         loadContactFormEdit(message);
        								break;
	...
	public void onLongClickItem(){
		lvMessages.setOnItemLongClickListener(new OnItemLongClickListener() {
            public boolean onItemLongClick(AdapterView<?> arg0, final View view, int position, long id) {
            	final int id_message = util.convertStringToInt(list.get(position).get("id"));
				util.confirm(ContactActivity.this, 
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
	        case android.R.id.home: if (currentPage == PAGE_CONTACT_LIST){
						    			return(super.onOptionsItemSelected(item));
						    		}
	        						redirectBack();
	        						return true;

	        case R.id.menu_new: loadContactFormNew();
	        					return true;

	        case R.id.menu_save: saveContactMessage();
	        					 return true;
