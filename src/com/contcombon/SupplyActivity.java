...
public class SupplyActivity extends BaseActivity {
  ...
  private Handler handler = new Handler() {
    ...
    public void handleMessage(Message msg) {
      ...
      switch (msg.what) {
        case CREATE_SUCCESS:
          util.showToast(getResources().getString(R.string.create_success), SupplyActivity.this);
          loadSupplyListPage();
          enableDrawer();
          break;
        case UPDATE_SUCCESS:
          util.showToast(getResources().getString(R.string.update_success), SupplyActivity.this);
          loadSupplyListPage();
          enableDrawer();
          break;
        case MSG_ERROR:
          util.showMessage(getResources().getString(R.string.error), errorMessage, SupplyActivity.this);
          break;
        case LIST:
          ...
          break;
        case EDIT_FORM:
          loadSupplyFormEdit(supply);
          break;
        case DELETE_SUCCESS:
          util.showToast(getResources().getString(R.string.delete_success), SupplyActivity.this);
          loadSupplyListPage();
          enableDrawer();
          break;
        case SPINNER_LIST:
          prepareSpinnerVehicles();
  ...
  public void onLongClickItem() {
    lvSupplies.setOnItemLongClickListener(new OnItemLongClickListener() {
      ...
      public boolean onItemLongClick(AdapterView<?> arg0, final View view, int position, long id) {
        final int id_supply = util.convertStringToInt(list.get(position).get("id").toString());
        util.confirm(SupplyActivity.this, getResources().getString(R.string.confirmation), getResources().getString(R.string.delete_confirmation), getResources().getString(R.string.yes),
  ...
  protected Dialog onCreateDialog(int id) {
    switch (id) {
      case DATE_DIALOG_ID:
        return new DatePickerDialog(this, mDateSetListener, dpYear, dpMonth, dpDay);
    }
    return null;
  }
  ...
  protected void onPrepareDialog(int id, Dialog dialog) {
    switch (id) {
      case DATE_DIALOG_ID:
        ((DatePickerDialog) dialog).updateDate(dpYear, dpMonth, dpDay);
    }
  }

  private void updateTvDate() {
    tvDate = (TextView) findViewById(R.id.tvDate);
    date = new StringBuilder().append(dpDay).append("/").append(dpMonth + 1).append("/") // Month
                                                                                         // is
                                                                                         // 0
                                                                                         // based
                                                                                         // so
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    switch (item.getItemId()) {
      case android.R.id.home:
        if (currentPage == PAGE_SUPPLY_LIST) {
